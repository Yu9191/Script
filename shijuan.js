...
if (jpgMatch) {
    ...
    if (isUserAgent) {
        // 检查是否含有 X-Playback-Session-Id 头部（QuanX是大写X-P-S-I）
        const isM3U8 = headers.hasOwnProperty("x-playback-session-id") || headers.hasOwnProperty("X-Playback-Session-Id");

        if (isM3U8) {
            // 显示通知，通知内容为捕获成功信息和视频链接
            $.msg("视频链接捕获成功", "点击此通知在线观看", "通知详细", url);
        } else {
            $.msg("你在浏览器不会捕获", "你可以下载这个视频");
        }
    } else {
        console.log("User-Agent 头部不存在或不包含 '小白学习盒子'，不显示通知");
    }
}
